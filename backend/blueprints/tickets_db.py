...
def init_db():
    ...
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS projects (
            ...
            audit_apps TEXT DEFAULT ''
        )
    ''')

    # Migrate: add ollama columns if missing (for existing databases)
    existing_cols = {row[1] for row in cursor.execute('PRAGMA table_info(projects)').fetchall()}
    if 'ollama_enabled' not in existing_cols:
        cursor.execute("ALTER TABLE projects ADD COLUMN ollama_enabled INTEGER DEFAULT 0")
    if 'ollama_model' not in existing_cols:
        cursor.execute("ALTER TABLE projects ADD COLUMN ollama_model TEXT DEFAULT 'mistral'")
    if 'audit_apps' not in existing_cols:
        cursor.execute("ALTER TABLE projects ADD COLUMN audit_apps TEXT DEFAULT ''")
    ...
    try:
        conn.execute(
            '''INSERT INTO projects (
                id, name, description, owner, members, columns, color, 
                created, updated, copilot_enabled, localai_enabled, freemodel_enabled,
                ollama_enabled, ollama_model, audit_apps
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
            (
                ...
                1 if data.get('ollama_enabled') else 0,
                data.get('ollama_model', 'mistral'),
                data.get('audit_apps', '')
            )
        )
        conn.commit()
