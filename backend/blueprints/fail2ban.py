...
def run_command(cmd):
    allowed_cmds = ['fail2ban-client', 'status', 'set', 'get']
    if not all(c in allowed_cmds for c in cmd):
        raise ValueError("Invalid command")

    try:
        result = subprocess.run(['sudo', '-n'] + cmd, capture_output=True, text=True, timeout=5)
