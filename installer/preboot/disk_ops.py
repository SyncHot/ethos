...
def _btrfs_opts(part_path, subvol="@data"):
    ...
    All data partitions get nofail so the system boots cleanly even when
    the data disk has a transient error or is being formatted.
    USB/removable disks additionally get x-systemd.automount for hot-plug.
    Internal disks get a longer device-timeout (60 s) to survive slow NVMe
    spin-up; USB gets the shorter 30 s.
    """
    if subvol == "@data":
    ...
    if _is_usb_part(part_path):
        ...
        # Internal NVMe/SATA data disk: nofail prevents black-screen boot hang
        # when btrfs doesn't come up cleanly; EthOS mounts it later if needed.
        opts += ",nofail,x-systemd.device-timeout=60"
    return opts
