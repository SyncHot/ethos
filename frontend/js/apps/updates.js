...
function renderUpdatesApp(body) {
        ...
        <div style="flex:1;overflow-y:auto;">
            ...
            <div class="upd-pane" id="upd-pane-settings" style="padding:24px;display:none;">
                <div style="display:flex;flex-direction:column;gap:18px;">
                    ...
                    <!-- GitHub Release — commit, push, create release -->
                    <div style="background:var(--bg-secondary);border:1px solid var(--border);border-radius:12px;padding:16px 18px;">
                        <div style="display:flex;align-items:center;gap:10px;margin-bottom:10px;">
                            <i class="fab fa-github" style="font-size:18px;color:var(--text);"></i>
                            <span style="font-size:14px;font-weight:700;">GitHub Release</span>
                        </div>
                        <div style="font-size:12px;color:var(--text-secondary);margin-bottom:14px;">
                            Commit, push i utwórz GitHub Release — wszystkie maszyny mogą się z niego zaktualizować.
                        </div>
                        <div style="display:flex;flex-direction:column;gap:10px;">
                            <div>
                                <label style="font-size:12px;font-weight:600;display:block;margin-bottom:4px;">GitHub PAT Token</label>
                                <div style="display:flex;gap:6px;">
                                    <input type="password" id="upd-gh-token" placeholder="${t('Wklej token (classic, scope: repo)')}" style="flex:1;padding:8px 10px;border-radius:8px;border:1px solid var(--border);background:var(--bg-primary);color:var(--text);font-size:12px;font-family:monospace;">
                                    <span id="upd-gh-token-status" style="font-size:11px;color:var(--text-secondary);align-self:center;white-space:nowrap;"></span>
                                </div>
                            </div>
                            <div>
                                <label style="font-size:12px;font-weight:600;display:block;margin-bottom:4px;">Repozytorium</label>
                                <div style="display:flex;gap:6px;">
                                    <input type="text" id="upd-gh-repo" placeholder="owner/repo" style="flex:1;padding:8px 10px;border-radius:8px;border:1px solid var(--border);background:var(--bg-primary);color:var(--text);font-size:12px;font-family:monospace;">
                                    <button class="btn btn-secondary" id="upd-gh-detect-btn" style="font-size:12px;padding:6px 10px;gap:4px;white-space:nowrap;">
                                        <i class="fas fa-search"></i> Auto-detect
                                    </button>
                                </div>
                            </div>
                            <div>
                                <label style="font-size:12px;font-weight:600;display:block;margin-bottom:4px;">Opis zmiany (changelog)</label>
                                <input type="text" id="upd-gh-message" placeholder="${t('np. Nowa funkcja X, fix Y')}" style="width:100%;padding:8px 10px;border-radius:8px;border:1px solid var(--border);background:var(--bg-primary);color:var(--text);font-size:12px;box-sizing:border-box;">
                            </div>
                            <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
                                <label style="font-size:12px;font-weight:600;">Bump wersji:</label>
                                <select id="upd-gh-bump" style="padding:6px 10px;border-radius:8px;border:1px solid var(--border);background:var(--bg-primary);color:var(--text);font-size:12px;">
                                    <option value="patch">patch (x.x.+1)</option>
                                    <option value="minor">minor (x.+1.0)</option>
                                    <option value="major">major (+1.0.0)</option>
                                </select>
                                <button class="btn btn-primary" id="upd-gh-release-btn" style="gap:6px;font-size:13px;margin-left:auto;">
                                    <i class="fab fa-github"></i> Commit, Push &amp; Release
                                </button>
                            </div>
                            <div id="upd-gh-result" style="display:none;font-size:12px;padding:8px 12px;border-radius:8px;"></div>
                        </div>
                    </div>
    ...
    function fillSettings() {
        ...
        // GitHub
        const ghRepo = $('#upd-gh-repo');
        const ghStatus = $('#upd-gh-token-status');
        if (ghRepo && config.github_repo) ghRepo.value = config.github_repo;
        if (ghStatus) {
            ghStatus.textContent = config.github_token_set ? '✓ token zapisany' : '';
            ghStatus.style.color = config.github_token_set ? '#22c55e' : 'var(--text-secondary)';
        }
    }
    ...
    async function saveConfig() {
        const payload = {
            ...
            github_repo: ($('#upd-gh-repo') || {}).value?.trim() || '',
        };
        const tokenVal = ($('#upd-gh-token') || {}).value?.trim();
        if (tokenVal) payload.github_token = tokenVal;
        try {
            const data = await api('/update/config', {
                method: 'PUT',
            ...
            if (data.success) {
                ...
                // Token was sent — update UI indicator
                if (tokenVal) {
                    config.github_token_set = true;
                    const ghStatus = $('#upd-gh-token-status');
                    if (ghStatus) { ghStatus.textContent = '✓ token zapisany'; ghStatus.style.color = '#22c55e'; }
                    const ghInput = $('#upd-gh-token');
    ...
    async function detectGitHubRepo() {
        const btn = $('#upd-gh-detect-btn');
        btn.disabled = true;
        btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i>';
        try {
            const data = await api('/update/github-detect-repo');
            if (data.repo) {
                $('#upd-gh-repo').value = data.repo;
                toast('Repo wykryte: ' + data.repo, 'success');
            } else {
                toast('Nie wykryto repo — wpisz ręcznie', 'warning');
            }
        } catch (e) {
            toast(t('Błąd: ') + e, 'error');
        }
        btn.disabled = false;
        btn.innerHTML = '<i class="fas fa-search"></i> Auto-detect';
    }

    async function publishGitHub() {
        const message = ($('#upd-gh-message') || {}).value?.trim();
        if (!message) { toast('Wpisz opis zmiany', 'warning'); return; }
        const bump = $('#upd-gh-bump')?.value || 'patch';

        if (!await confirmDialog(`Commit, push i GitHub Release (bump: ${bump})?\n\n"${message}"`)) return;

        const btn = $('#upd-gh-release-btn');
        const result = $('#upd-gh-result');
        btn.disabled = true;
        btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> W toku…';
        if (result) { result.style.display = 'none'; }

        // Switch to log tab so user sees progress
        body.querySelectorAll('.upd-nav').forEach(n => n.classList.remove('active'));
        body.querySelectorAll('.upd-pane').forEach(p => p.style.display = 'none');
        const logNav = body.querySelector('.upd-nav[data-tab="log"]');
        if (logNav) logNav.classList.add('active');
        $('#upd-pane-log').style.display = '';

        addLog('── GitHub Release ──');
        try {
            const data = await api('/update/github-release', {
                method: 'POST',
                body: { message, bump },
            });
            if (data.error) {
                ...
                addLog(data.error, true);
                btn.disabled = false;
                btn.innerHTML = '<i class="fab fa-github"></i> Commit, Push & Release';
            }
            // Result shown via SocketIO event github_release_done
        } catch (e) {
            toast(t('Błąd: ') + e, 'error');
            addLog(t('Błąd: ') + e, true);
            btn.disabled = false;
    ...
    function setupSocket() {
        ...
        NAS.socket.on('github_release_done', (data) => {
            const btn = $('#upd-gh-release-btn');
            if (btn) {
                btn.disabled = false;
                btn.innerHTML = '<i class="fab fa-github"></i> Commit, Push & Release';
            ...
            if (result) {
                ...
                if (data.ok) {
                    ...
                    result.innerHTML = `<i class="fas fa-check-circle"></i> Release <b>v${data.version}</b> gotowy! 
                        ${data.url ? `<a href="${data.url}" target="_blank" style="color:#22c55e;margin-left:8px;">Otwórz na GitHub ↗</a>` : ''}`;
                    toast(`Release v${data.version} gotowy!`, 'success');
                    ...
                    result.innerHTML = `<i class="fas fa-times-circle"></i> ${data.error || 'Błąd'}`;
                    toast(data.error || 'Błąd release', 'error');
                }
            }
        });
    ...
    $('#upd-gh-release-btn').onclick = publishGitHub;
    $('#upd-gh-detect-btn').onclick = detectGitHubRepo;
