...
function renderResourcesApp(body) {
    ...
    const pct = (val) => val != null ? val.toFixed(1) + '%' : '—';
    ...
    function updateAll(data) {
        lastData = data;
        const conn = $('#res-conn');
        if (conn) conn.innerHTML = '<i class="fas fa-circle res-conn-dot res-conn-online"></i> Online';
    ...
    function renderOverview(el, data) {
        ...
        <div class="res-grid-4">
            <!-- CPU Card -->
            <div class="res-card">
                <div class="res-card-hdr"><i class="fas fa-microchip"></i> ${t('CPU')}</div>
            ...
            <!-- RAM Card -->
            <div class="res-card">
                <div class="res-card-hdr"><i class="fas fa-memory"></i> ${t('RAM')}</div>
            ...
            <!-- GPU Card -->
            <div class="res-card">
                <div class="res-card-hdr"><i class="fas fa-tv"></i> ${t('GPU')}</div>
                <div class="res-big-val">${gpu.load != null ? pct(gpu.load) : '<span class="res-gpu-nodetect">' + t('Brak') + '</span>'}</div>
                ${bar(gpu.load, '#f59e0b')}
                <div class="res-card-stats">
                    <span>${gpu.name || '<span class="res-gpu-hint">' + t('Nie wykryto') + '</span>'}</span>
                    <span>${gpu.temperature != null ? gpu.temperature+'°C' : ''}</span>
                </div>
            </div>

            <!-- Network Card (Improved) -->
            <div class="res-card">
                <div class="res-card-hdr"><i class="fas fa-network-wired"></i> ${t('Sieć')}</div>
                <div class="res-net-speeds">
                    <div class="res-net-item down"><i class="fas fa-arrow-down"></i> ${fmtSpeed(net.speed_download)}</div>
                    <div class="res-net-item up"><i class="fas fa-arrow-up"></i> ${fmtSpeed(net.speed_upload)}</div>
                </div>
                <div class="res-card-stats">
        ...
        <div class="res-grid-2 res-mt-md">
            <!-- Disks -->
            <div class="res-card">
                <div class="res-card-hdr"><i class="fas fa-hdd"></i> ${t('Dyski')}</div>
                ...
                }).join('') || '<p class="res-text-muted">' + t('Brak danych') + '</p>'}</div>
            </div>

            <!-- Top Processes -->
            <div class="res-card">
                <div class="res-card-hdr"><i class="fas fa-list-alt"></i> ${t('Top procesy (CPU')}</div>
                <table class="res-proc-table">
                    <thead><tr><th>PID</th><th>Nazwa</th><th>CPU</th><th>RAM</th></tr></thead>
                    <tbody>${procs.map(p => `
    ...
    function renderSystem(el) {
        ...
        <div class="res-grid-2 res-mt-md">
            <div class="res-card">
                ...
                <div class="res-sys-info">
                    ...
                    ${infoRow('fa-th', t('Rdzenie / Wątki'), cpu.cores ? cpu.cores + ' / ' + (cpu.threads_per_core || 1) : '')}
        ...
        <div class="res-card res-mt-md">
            ...
            <table class="res-proc-table">
                <thead><tr><th>${t('Slot')}</th><th>${t('Rozmiar')}</th><th>${t('Typ')}</th><th>${t('Prędkość')}</th><th>${t('Producent')}</th></tr></thead>
                <tbody>${dimms.map(d => `
                    <tr><td>${d.locator||'—'}</td><td>${d.size||'—'}</td><td>${d.type||'—'}</td><td>${d.speed||'—'}</td><td>${d.manufacturer||'—'}</td></tr>
                `).join('')}</tbody>
            </table>
        </div>` : ''}
        `;
    ...
    function renderCPU(el, data) {
        ...
        <div class="res-info-bar">
            ...
            <span>${t('Częstotliwość:')} ${cpu.frequency_current ? (cpu.frequency_current/1000).toFixed(2)+' GHz' : '—'}</span>
        ...
        <div class="res-card res-mt-md">
            ...
            <table class="res-proc-table">
                <thead><tr><th>PID</th><th>Nazwa</th><th>CPU %</th><th>RAM</th><th>Status</th><th>${t('Użytkownik')}</th></tr></thead>
    ...
    function renderRAM(el, data) {
        ...
        <div class="res-card res-mt-md">
            ...
            <table class="res-proc-table">
                <thead><tr><th>PID</th><th>Nazwa</th><th>RAM %</th><th>RAM</th><th>CPU %</th><th>Status</th></tr></thead>
    ...
    function renderGPU(el, data) {
        ...
        if (!gpus.length) {
            if (!el._gpuDetected) {
                el.innerHTML = `<div class="res-empty"><i class="fas fa-tv res-empty-icon"></i><p class="res-empty-title">${t('Brak wykrytego GPU')}</p><p class="res-empty-detect">${t('Wykrywanie sprzętu…')}</p></div>`;
                ...
                }).catch(() => _renderGpuNoDriver(el, {cards: []}));
            } else {
        ...
        <div class="res-card">
            ...
            <div class="res-grid-2">
                <div><span>${t('Obciążenie:')}</span>${bar(g.load, '#f59e0b')} <span class="res-big-val">${pct(g.load)}</span></div>
                <div><span>VRAM: ${fmt(g.memory_used)} / ${fmt(g.memory_total)}</span>${bar(g.memory_total ? ((g.memory_used||0)/g.memory_total*100) : 0, '#ef4444')}</div>
            </div>
    ...
    function _renderGpuNoDriver(el, hw) {
        ...
        if (!cards.length) {
            el.innerHTML = `<div class="res-empty"><i class="fas fa-tv res-empty-icon"></i><p class="res-empty-title">${t('Brak wykrytego GPU')}</p><p class="res-empty-desc">${t('Nie wykryto karty graficznej w systemie.')}</p></div>`;
            return;
        }

        el.innerHTML = cards.map((c, i) => `
        <div class="res-card res-mb-md">
            <div class="res-card-hdr"><i class="fas ${vendorIcons[c.vendor]||vendorIcons.unknown}" style="color:${vendorColors[c.vendor]||vendorColors.unknown}"></i> ${c.name || 'Karta graficzna'}</div>
            <div class="res-gpu-card-body">
                <div class="res-flex-row-mb"><span class="res-label">Producent:</span><span class="res-vendor-name">${c.vendor}</span></div>
                <div class="res-flex-row-mb"><span class="res-label">Pakiety:</span><span class="res-mono-sm">${(c.packages||[]).join(', ') || '—'}</span></div>
                ${c.driver_installed ? `<div class="res-gpu-status-ok"><i class="fas fa-check-circle res-gpu-status-ok-icon"></i><span class="res-gpu-status-ok-text">${t('Sterowniki zainstalowane')}</span></div>` : (c.packages_installed && c.reboot_required) ? `<div class="res-gpu-status-warn"><i class="fas fa-exclamation-triangle res-gpu-status-warn-icon"></i><span class="res-gpu-status-warn-text">${t('Sterowniki zainstalowane, wymagany restart systemu')}</span></div><div class="res-reboot-area"><button onclick="window._resReboot(this)" class="res-reboot-btn"><i class="fas fa-redo"></i> ${t('Uruchom ponownie')}</button></div>` : c.install_cmd ? `<div id="gpu-install-area-${i}" class="res-mt-md"><button onclick="window._gpuInstallDriver(${i})" class="res-gpu-install-btn res-gpu-install-btn-base" style="background:${vendorColors[c.vendor]||'#3b82f6'}"><i class="fas fa-download"></i> ${t('Pobierz i zainstaluj sterowniki')}</button></div>` : `<p class="res-no-install">${t('Brak automatycznej instalacji dla tego producenta.')}</p>`}
            </div>
        </div>`).join('');
        ...
        window._gpuInstallDriver = (cardIndex) => {
            ...
            if (!area) return;
            _gpuInstalling = true;
            area.innerHTML = `<div class="res-install-wrap"><div class="res-install-status-row"><i class="fas fa-spinner fa-spin res-install-spinner"></i><span id="gpu-install-status" class="res-install-status">${t('Rozpoczynanie…')}</span></div><div class="res-install-bar-bg"><div id="gpu-install-bar" class="res-install-bar-fill"></div><span id="gpu-install-pct" class="res-install-bar-pct">0%</span></div><div id="gpu-install-detail" class="res-install-detail"></div></div>`;
            ...
            _gpuInstallListener = (ev) => {
                ...
                if (ev.phase === 'done') {
                    ...
                    if (ev.reboot_required) {
                        ...
                        rebootArea.innerHTML = `<button onclick="window._resReboot(this)" class="res-reboot-btn"><i class="fas fa-redo"></i> Uruchom ponownie</button>`;
            ...
            api('/resources/gpu/install', { method: 'POST', body: JSON.stringify({card_index: cardIndex}) }).then(res => {
                if (res.error) {
                    const statusEl = document.getElementById('gpu-install-status');
    ...
    function renderDisks(el, data) {
        const disks = data.disks || [];
        const diskCat = (d) => d.is_usb ? 'usb' : (d.mountpoint.startsWith('/media/') ? 'ext' : 'sys');
        const diskIcon = (d) => { const c = diskCat(d); if (c === 'usb') return 'fa-usb'; if (c === 'ext') return 'fa-hdd'; return d.mountpoint === '/' ? 'fa-server' : 'fa-hdd'; };
        const diskIconColor = (d) => { const c = diskCat(d); if (c === 'usb') return '#a78bfa'; if (c === 'ext') return '#f59e0b'; return '#3b82f6'; };
        const diskLabel = (d) => d.label || (d.mountpoint === '/' ? 'System /' : d.mountpoint.split('/').pop() || d.mountpoint);
        const diskColor = (p) => p > 90 ? '#ef4444' : p > 70 ? '#eab308' : '#10b981';
        ...
        function renderCard(d) {
            ...
            <div class="ddisk-card">
                ...
                <div class="ddisk-info">
                    <div class="ddisk-name">${diskLabel(d)} ${d.model ? `<span class="res-disk-model">${d.model}</span>` : ''}</div>
                    ...
                    <div class="ddisk-sizes"><span><b>${fmt(d.used)}</b> ${t('zajęte')}</span><span><b>${fmt(d.free)}</b> wolne</span><span>z <b>${fmt(d.total)}</b></span></div>
                </div>
            </div>`;
        }
        ...
        el.innerHTML = `<h3><i class="fas fa-hdd"></i> ${t('Dyski i pamięć masowa')}</h3>` +
            (sys.length ? `<div class="ddisk-group"><div class="ddisk-group-title"><i class="fas fa-server"></i> Dyski systemowe</div><div class="ddisk-group-cards">${sys.map(renderCard).join('')}</div></div>` : '') +
            (ext.length ? `<div class="ddisk-group"><div class="ddisk-group-title"><i class="fas fa-hdd"></i> ${t('Dyski zewnętrzne')}</div><div class="ddisk-group-cards">${ext.map(renderCard).join('')}</div></div>` : '') +
            (usb.length ? `<div class="ddisk-group"><div class="ddisk-group-title"><i class="fas fa-usb"></i> Dyski USB</div><div class="ddisk-group-cards">${usb.map(renderCard).join('')}</div></div>` : '') +
    ...
    function renderSMART(el, data) {
        ...
        if (!smart.length) {
            el.innerHTML = `<div class="res-empty"><i class="fas fa-heartbeat res-empty-icon"></i><p class="res-empty-title">${t('Brak danych S.M.A.R.T.')}</p><p class="res-empty-desc">${t('Może być wymagana konfiguracja smartmontools lub brak obsługiwanych dysków.')}</p></div>`;
            return;
        }
        ...
        el.innerHTML = `<h3><i class="fas fa-heartbeat"></i> ${t('Zdrowie dysków (S.M.A.R.T.)')}</h3>` + smart.map(d => {
            ...
            <div class="res-card res-mb-md">
                <div class="res-card-hdr">
                    <div><i class="fas fa-hdd" style="color:#a78bfa"></i> ${d.model} <span class="res-mono-sm" style="opacity:0.7">(${d.device})</span></div>
                    <div style="margin-left:auto;color:${healthColor};font-weight:bold"><i class="fas ${healthIcon}"></i> ${d.health}</div>
                </div>
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;padding:15px 0;">
                    <div class="res-kv-row"><span class="res-label">Serial Number:</span><span class="res-val">${d.serial}</span></div>
                    <div class="res-kv-row"><span class="res-label">Temperatura:</span><span class="res-val" style="color:${tempColor}">${d.temperature ? d.temperature + '°C' : '—'}</span></div>
                    <div class="res-kv-row"><span class="res-label">Power On Hours:</span><span class="res-val">${d.power_on_hours} h</span></div>
                    <div class="res-kv-row"><span class="res-label">${t('Żywotność (SSD):')}</span><span class="res-val">${d.remaining_life >= 0 ? pct(d.remaining_life) : '—'}</span></div>
                </div>
    ...
    function renderNetwork(el, data) {
        ...
        <div class="res-card res-mt-md">
            ...
            <table class="res-proc-table">
                <thead><tr><th>Interfejs</th><th>IP</th><th>↓ Speed</th><th>↑ Speed</th></tr></thead>
                <tbody>${ifaces.map(i => `
                    <tr><td>${i.name}</td><td>${i.addresses ? i.addresses.join(', ') : '—'}</td><td>${fmtSpeed(i.speed_download)}</td><td>${fmtSpeed(i.speed_upload)}</td></tr>
                `).join('')}</tbody>
            </table>
        </div>` : ''}`;
    }
    ...
    function renderProcesses(el, data) {
        ...
        <div class="res-card res-proc-card">
            <table class="res-proc-table">
                <thead><tr><th>PID</th><th>Nazwa</th><th>CPU %</th><th>RAM %</th><th>RAM</th><th>Status</th><th>${t('Użytkownik')}</th><th>Akcja</th></tr></thead>
                <tbody>${filtered.slice(0, 100).map(p => `
                    <tr><td>${p.pid}</td><td title="${p.cmdline||''}">${p.name}</td><td>${pct(p.cpu_percent)}</td><td>${pct(p.memory_percent)}</td><td>${fmt(p.memory_rss)}</td><td>${p.status||''}</td><td>${p.username||''}</td>
    ...
    function renderUSB(el, data) {
        ...
        const classIcons = {'03': 'fa-keyboard', '07': 'fa-print', '08': 'fa-hdd', '09': 'fa-sitemap', '0e': 'fa-video', 'e0': 'fa-broadcast-tower', '01': 'fa-music', '06': 'fa-camera'};
        ...
        el.innerHTML = `<h3><i class="fas fa-usb"></i> ${t('Urządzenia USB')} (${devices.length})</h3>` +
            (devices.length ? devices.map(d => {
                ...
                <div class="res-card res-mt-sm">
                    <div class="res-card-hdr res-usb-hdr"><i class="fas ${icon} res-usb-icon"></i><span>${d.product || t('Nieznane urządzenie')}</span>${d.device_class_name ? `<span class="res-usb-badge">${d.device_class_name}</span>` : ''}</div>
                    <div class="res-info-bar res-usb-info">
                        ${d.manufacturer ? `<span><i class="fas fa-industry res-icon-muted"></i>${d.manufacturer}</span>` : ''}
                        ...
                        ${d.speed ? `<span><i class="fas fa-tachometer-alt res-icon-muted"></i>${d.speed} Mbit/s</span>` : ''}
                        ${d.power ? `<span><i class="fas fa-bolt res-icon-muted"></i>${d.power}</span>` : ''}
                    </div>
                </div>`;
    ...
    (async () => {
        try {
            const [d, hw] = await Promise.all([api('/resources/all'), api('/hardware/profile')]);
            hwProfile = hw;
            updateAll(d);
        } catch(e) {
